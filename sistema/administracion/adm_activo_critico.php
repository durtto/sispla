...
/******************************************INICIO**StoreActivo******************************************/     
  ...
  var storeActivo = new Ext.data.JsonStore({
   ...
   var expanderActivo = new Ext.ux.grid.RowExpander({
        tpl : new Ext.Template(
            '<p><b>Codigo</b> {co_activo}</p>',
            '<p><b>Nombre:</b> {nb_activo}</p>',
            '<p><b>Descripcion:</b> {tx_telefono_oficina}</p>',
            '<p><b>Vulnerable:</b> {bo_vulnerable}</p>',
            '<p><b>Ubicaci&oacute;n:</b> {co_ubicacion}</p>'
        )
    });
/*****************************************FIN****StoreActivo*****************************************/
...
function selPersona(){
storePersona.load({params: { start: 0, limit: 50, accion:"refrescar", interfaz: "interfaz_persona.php"}});
	if(!winPersona){
